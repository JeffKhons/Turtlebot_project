...
while (abs(alpha) >= 0.1)
  ...
  % 進行 AMCL 自主定位運算  
    ...
    rec.PoseRotateforAMCL(idx, :) = pose;
    rec.ScansRotateforAMCL(idx, :) = transScan;
    idx = idx +1;
...
while(goalDist >= 0.05)
    ...
    % 進行 AMCL 自主定位運算
    rec.PoseforAMCL(idy, :) = pose;
    rec.ScansforAMCL(idy, :) = transScan;
%     [isUpdated, estimatedPose, estimatedCovariance] = amcl(pose, transScan);
%     estimatedPose
%     rec.estimatedPose(idy, :) = estimatedPose;
